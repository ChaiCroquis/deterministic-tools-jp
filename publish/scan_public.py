...
PATTERNS = {
    ...
    # データを預かる形(代行)は書かない(2026-09-20 決定: 道具そのものを公開して渡す。個別対応はカスタマイズのみ)
    # 否定形(「代行はしない」「データは預かりません」「預かることはしない」)は方針の明示なので許容
    "custody word": re.compile(r"代行(?![はをも]?しない|(?:はしません))|データ[をは]預か(?!(?:ることは)?(?:しない|しません)|りません|らない)|窓口は準備中"),
}
